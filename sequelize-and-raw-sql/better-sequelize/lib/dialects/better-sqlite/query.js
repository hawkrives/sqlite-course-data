...
class Query extends AbstractQuery {
  ...
  async run(sql, parameters) {
    ...
    const executeSql = () => {
      ...
      if (method === 'exec') {
        try {
          // exec does not support bind parameter
          let results = this.database.exec(this.sql);
          return afterExecute(results)
        } catch (err) {
          err.sql = query.sql;
          throw query.formatError(err);
        }
      }

      try {
        let results = this.database.prepare(this.sql)[method](parameters);
        return afterExecute(results)
      } catch (err) {
        err.sql = query.sql;
        throw query.formatError(err);
    ...
    if (this.getDatabaseMethod() === 'all') {
      ...
      tableNames.forEach(tableName => {
        ...
        let results = this.database.pragma('table_info(`' + tableName + '`)')
  ...
  getDatabaseMethod() {
    if (this.isUpsertQuery()) {
      ...
      || this.sql.toUpperCase().includes('CREATE TEMPORARY TABLE')
      || this.sql.toUpperCase().includes('DROP TABLE')
      || this.sql.toUpperCase().includes('CREATE TABLE')
      || this.sql.toUpperCase().includes('BEGIN DEFERRED TRANSACTION')
      || this.sql.toUpperCase().includes('ROLLBACK')
      || this.sql.toUpperCase().includes('COMMIT')
