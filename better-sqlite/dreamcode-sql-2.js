...
function insertCourse(sql, course) {
	...
	departments.forEach(name => {
		sql.run`INSERT OR IGNORE INTO department (name) VALUES (${name})`
		let {id} = sql.get`SELECT id FROM department WHERE name = ${name}`
		sql.run`INSERT OR IGNORE INTO course_department (course_id, department_id) VALUES (${cid}, ${id})`
	})

	gereqs.forEach(name => {
		sql.run`INSERT OR IGNORE INTO gereq (name) VALUES (${name})`
		let {id} = sql.get`SELECT id FROM gereq WHERE name = ${name}`
		sql.run`INSERT OR IGNORE INTO course_gereq (course_id, gereq_id) VALUES (${cid}, ${id})`
	})

	instructors.forEach(name => {
		sql.run`INSERT OR IGNORE INTO instructor (name) VALUES (${name})`
		let {id} = sql.get`SELECT id FROM instructor WHERE name = ${name}`
		sql.run`INSERT OR IGNORE INTO course_instructor (course_id, instructor_id) VALUES (${cid}, ${id})`
	})

	locations.forEach(name => {
		sql.run`INSERT OR IGNORE INTO location (name) VALUES (${name})`
		let {id} = sql.get`SELECT id FROM location WHERE name = ${name}`
		sql.run`INSERT OR IGNORE INTO course_location (course_id, location_id) VALUES (${cid}, ${id})`
	})

	notes.forEach(content => {
		sql.run`INSERT OR IGNORE INTO note (content) VALUES (${content})`
		let {id} = sql.get`SELECT id FROM note WHERE content = ${content}`
		sql.run`INSERT OR IGNORE INTO course_note (course_id, note_id) VALUES (${cid}, ${id})`
	})

	description.forEach(content => {
		sql.run`INSERT OR IGNORE INTO description (content) VALUES (${content})`
		let {id} = sql.get`SELECT id FROM description WHERE content = ${content}`
		sql.run`INSERT OR IGNORE INTO course_description (course_id, description_id) VALUES (${cid}, ${id})`
	})
	...
	times.forEach(timestring => {
		...
		sql.run`INSERT OR IGNORE INTO time (days, start, end) VALUES (${days}, ${start}, ${end})`
		let {id} = sql.get`
			SELECT id FROM time WHERE days = ${days} AND start = ${start} AND end = ${end}
		`
		sql.run`INSERT OR IGNORE INTO course_time (course_id, time_id) VALUES (${cid}, ${id})`
	})

	if (prerequisites) {
		sql.run`INSERT OR IGNORE INTO prerequisite (content) VALUES (${prerequisites})`
		let {id} = sql.get`
			SELECT id FROM prerequisite WHERE content = ${prerequisites}
		`
		sql.run`INSERT OR IGNORE INTO course_prerequisite (course_id, prerequisite_id) VALUES (${cid}, ${id})`
	}
}
