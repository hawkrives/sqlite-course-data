...
async function makeTables(db) {
	const makeIdNameTable = ({ name }) => {
		let indexName = `${name}_name_uindex`
		sql.run`DROP TABLE IF EXISTS department`
		sql.run`CREATE TABLE IF NOT EXISTS department (id integer PRIMARY KEY AUTOINCREMENT, name text NOT NULL)`
		sql.run`DROP INDEX IF EXISTS department_name_uindex`
		sql.run`CREATE UNIQUE INDEX department_name_uindex ON department (name)`
	}
	sql.run`DROP TABLE IF EXISTS department`
	sql.run`CREATE TABLE IF NOT EXISTS department (id integer PRIMARY KEY AUTOINCREMENT, name text NOT NULL)`
	sql.run`DROP INDEX IF EXISTS department_name_uindex`
	...
	sql.run`DROP TABLE IF EXISTS instructor`
	sql.run`CREATE TABLE IF NOT EXISTS instructor (id integer PRIMARY KEY AUTOINCREMENT, name text NOT NULL)`
	sql.run`DROP INDEX IF EXISTS instructor_name_uindex`
	sql.run`CREATE UNIQUE INDEX instructor_name_uindex ON instructor (name)`

	sql.run`DROP TABLE IF EXISTS gereq`
	sql.run`CREATE TABLE IF NOT EXISTS gereq (id integer PRIMARY KEY AUTOINCREMENT, name text NOT NULL)`
	sql.run`DROP INDEX IF EXISTS gereq_name_uindex`
	sql.run`CREATE UNIQUE INDEX gereq_name_uindex ON gereq (name)`

	sql.run`DROP TABLE IF EXISTS location`
	sql.run`CREATE TABLE IF NOT EXISTS location (id integer PRIMARY KEY AUTOINCREMENT, name text NOT NULL)`
	sql.run`DROP INDEX IF EXISTS location_name_uindex`
	sql.run`CREATE UNIQUE INDEX location_name_uindex ON location (name)`

	sql.run`DROP TABLE IF EXISTS time`
	sql.run`
	...
	`
	sql.run`DROP INDEX IF EXISTS time_days_start_end_uindex`
	sql.run`CREATE UNIQUE INDEX time_days_start_end_uindex ON time (days, start, end)`

	sql.run`DROP TABLE IF EXISTS description`
	sql.run`CREATE TABLE IF NOT EXISTS description (id integer PRIMARY KEY AUTOINCREMENT, content text NOT NULL)`
	sql.run`DROP INDEX IF EXISTS description_content_uindex`
	sql.run`CREATE UNIQUE INDEX description_content_uindex ON description (content)`

	sql.run`DROP TABLE IF EXISTS note`
	sql.run`CREATE TABLE IF NOT EXISTS note (id integer PRIMARY KEY AUTOINCREMENT, content text NOT NULL)`
	sql.run`DROP INDEX IF EXISTS note_content_uindex`
	sql.run`CREATE UNIQUE INDEX note_content_uindex ON note (content)`

	sql.run`DROP TABLE IF EXISTS prerequisite`
	sql.run`CREATE TABLE IF NOT EXISTS prerequisite (id integer PRIMARY KEY AUTOINCREMENT, content text NOT NULL)`
	sql.run`DROP INDEX IF EXISTS prerequisite_content_uindex`
	sql.run`CREATE UNIQUE INDEX prerequisite_content_uindex ON prerequisite (content)`

	sql.run`DROP TABLE IF EXISTS course`
	sql.run`
