...
function main() {
	...
	asTransaction(() => {
		data.forEach(course => {
			...
			let {lastInsertROWID: courseId} = db.run`
				INSERT INTO course (clbid, credits, crsid, level, name, number, pn, section, status, title, type, year, semester)
				VALUES (${clbid}, ${credits}, ${crsid}, ${String(level)}, ${name}, ${String(number)}, ${pn ? 1 : 0}, ${section}, ${status}, ${title}, ${type}, ${year}, ${semester})
			`

			let {departments=[], gereqs=[], instructors=[], locations=[], notes=[], descriptions=[], times=[]} = course

			departments.forEach(department => {
				db.run`INSERT OR IGNORE INTO department (name) VALUES (${department})`
				let {id: departmentId} = db.get`SELECT id FROM department WHERE name = ${department}`
				db.run`INSERT OR IGNORE INTO course_note (course_id, note_id) VALUES (${courseId}, ${departmentId})`
			})
